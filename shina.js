...
mongoose.connect('mongodb+srv://1001:gamerbabo123@cluster0.3ye9b.mongodb.net/invitesanta?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${member} adlı üye sunucumuza katıldı! Kullanıcıyı davet eden: ${inviter.id == member.guild.id ? member.guild.name : inviter} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davete ulaştı)`).catch(err => {});
        });
      } else {
        inviterData.fake++
      ...
      Database.findOne({ guildID: member.guild.id, userID: memberData.inviterID }, async (err, inviterData) => {
        if (!inviterData) {
         let newInviter = new Database({
          ...
          inviterData.save().then(x => {
            if (inviteChannel) inviteChannel.send(`\`${member.user.tag}\` adlı kullanıcı aramızdan ayrıldı! ${inviter.tag ? `Kullanıcıyı davet eden: ${inviter.id == member.guild.id ? member.guild.name : inviter} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** daveti kaldı)` : `Davet eden bulunamadı!`}`).catch(err => {});
          });
        };
      });
    } else {
      if (inviteChannel) inviteChannel.send(`\`${member.user.tag}\` adlı kullanıcı aramızdan ayrıldı! Davet eden bulunamadı!`).catch(err => {});
    };
  });
});
...
client.login(process.env.botToken).then(c => console.log(`${client.user.tag} olarak giriş yapıldı!`)).catch(err => console.error("Bota giriş yapılırken başarısız olundu!"));
